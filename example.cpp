 ...
 *
 * bpStepsWriteRead.cpp  Simple example of writing and reading data through
 * ADIOS2 BP engine with multiple simulations steps for every IO step.
 *
...
void update_array(std::vector<float>& array, int val) {
  std::transform(array.begin(), array.end(), array.begin(), [val](float v) -> float {
    return v + static_cast<float>(val);
  });
}
...
void reader(adios2::ADIOS&     adios,
  ...
  for (unsigned int step = 0; bpReader.BeginStep() == adios2::StepStatus::OK;
       ++step) {
